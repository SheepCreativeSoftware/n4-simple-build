...
const map = (inputValue: number, inputMin: number, inputMax: number, outputMin: number, outputMax: number) => {
	// eslint-disable-next-line no-extra-parens
	return ((inputValue - inputMin) * (outputMax - outputMin)) / ((inputMax - inputMin) + outputMin);
};

const showProgressBar = (header: string, current: number, maxNumber: number) => {
	...
	if(current < maxNumber) {
		...
		output += '>[';
		for(let index = 0; index < numberOfSigns; index++) output += '=';
		for(let index = 0; index < consoleWidth-numberOfSigns; index++) output += ' ';
		output += ']';
