...
const unzipFile = ({ zipFilePath, outputPath, findFile }: { zipFilePath: string, outputPath: string, findFile: string }): Promise<string|null> => {
	let realFullpath = null as string|null;
...
const unzipFileMultiple = async ({ zipFilePath, outputPath, findFiles }: {
	zipFilePath: string, outputPath: string, findFiles: string[],
}): Promise<string[]> => {
	const filePaths = [] as string[];
	try {
		const promiseResults = [];
		for(const findFile of findFiles) promiseResults.push(unzipFile({ findFile, outputPath, zipFilePath }));

		const results = await Promise.allSettled(promiseResults);
		for(const result of results) {
			if(result.status === 'rejected') throw new Error(result.reason);
			if(result.value) filePaths.push(result.value);
		}
